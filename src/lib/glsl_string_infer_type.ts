...
type InferAttributeLine<T extends string> = T extends `${
  ...
  ? {
      ...
      type: Split<T, "[">[0] extends keyof Minitype
        ? Minitype[Split<T, "[">[0]]
        : never;
      name: V;
    }
...
type InferUniformLine<T extends string> =
  T extends `uniform ${infer T extends `${keyof Minitype}${string}`} ${infer V};`
    ? {
        size: T extends `${keyof Minitype}[${infer SZ extends number}]`
          ? SZ
          : 1;
        type: Split<T, "[">[0] extends keyof Minitype
          ? Minitype[Split<T, "[">[0]]
...
export type InferAttribute<T extends string> = {
  [key in Trim<
    Split<T, "\n">[number]
  > as InferAttributeLine<key>["name"]]: InferAttributeLine<key>;
};
export type InferUniform<T extends string> = {
  [key in Trim<
    Split<T, "\n">[number]
  > as InferUniformLine<key>["name"]]: InferUniformLine<key>;
};
