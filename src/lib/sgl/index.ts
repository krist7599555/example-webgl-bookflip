import { keyBy, range, mapValues } from "lodash-es";
import type { ConditionalPick, Merge, PartialDeep, Simplify, RemoveIndexSignature } from "type-fest";
import { createProgram, createShader } from "../krgl/helper";
import type { InferGlslLines, InferSglTypes } from "../krgl/type_helper";
...
type GLVarType = keyof typeof GL_TYPE2MINIMAL;
...
type CCCC = RemoveIndexSignature<SglState["attributes"]>
...
export function create_sgl_program<VS extends string, FS extends string>(opt: {
  ...
  type $$ActiveAttribute = MergeValue<ConditionalPick<
    $$TYPE,
    { $$type_glsl_qualifier: "attribute" }
  >, typeof a[string]>;
  type $$ActiveUniform = MergeValue<ConditionalPick<
    $$TYPE,
    { $$type_glsl_qualifier: "uniform" }
  >, typeof u[string]>;
  ...
  const state = {
    ...
    _webgl_ref_vao: null,
    // @ts-ignore
    attributes: mapValues(a, i => ({ name: i.name, enabled: false })) as MergeValue<$$ActiveAttribute, { enabled: false }>,
  } satisfies SglState;
...
export function sgl_create_vertex_array<S extends SglState>(
  sgl: SglProgram<S>
) {
